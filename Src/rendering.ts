...
const RenderScene = (ps: { [id: string] : Point }, ls: { [id: string] : Line }, ss: { [id: string] : Shape }, pCs: PointConstraint[], lCs: LineConstraint[]) => {
    ...
    let pointExpressions: Desmos.ExpressionState[] = [];
    ...
    for (const id in shapes) {
        ...
        if (shape.type == "rectangle") {
            ...
            const centerX: Desmos.ExpressionState = { id: CxID, latex: `${CxID} = ${Cx}` };
            const centerY: Desmos.ExpressionState = { id: CyID, latex: `${CyID} = ${Cy}` };
            const radius: Desmos.ExpressionState = { id: CrID, latex: `${CrID} = ${Cr}` };
            const equation: Desmos.ExpressionState = { id: circleID, latex: `\\left(x-${CxID}\\right)^{2}+\\left(y-${CyID}\\right)^{2}=${CrID}^{2}` };
    ...
    for (const id in lines) {
        ...
        if (line.gradient == undefined) {
            ...
            const gradientReadOnly: Desmos.ExpressionState = { id: `M_{${id}}`, latex: latex };
    ...
    const pointDisplays: Desmos.ExpressionState[] = [];
    for (const id in points) {
        ...
        if (id == id.toUpperCase()) {
            //This is a point generated by a line constructed with a gradient, so don't show it
            p.showLabel = false;
            p.hidden = true;
        }
